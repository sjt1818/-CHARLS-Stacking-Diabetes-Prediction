...
class ResultSaver:
    @staticmethod
    def save_results(model, processor, results, importance, stratified_results, save_dir, y_test):
        """保存模型和所有结果，接收y_test作为参数"""
...
# ========== 主函数 ==========
def main(load_existing_model=True):
    setup_environment()
    base_dir = os.getcwd()
    save_dir = os.path.join(base_dir, 'svm_optimized_results')
    os.makedirs(save_dir, exist_ok=True)
    model_path = os.path.join(save_dir, 'svm_best_model.pkl')
    ...
    # 如果选择加载现有模型且模型存在，则直接加载
    if load_existing_model and svm_model.load_model(model_path):
        print("使用现有模型进行评估，跳过训练步骤")
    else:
        # 否则进行训练
    ...
    # 7. 保存结果 - 将y_test作为参数传入
    ResultSaver.save_results(
        svm_model, processor, results, importance, stratified_results, save_dir, y_test
    )


if __name__ == "__main__":
    # 设置load_existing_model=True可加载已训练模型，避免重复训练
