...
DROP_FULL_MISSING_COLS = ["vgactx_c"]  # 初始已知完全缺失特征
# 简化映射：仅适配0、1、空（空值后续填充为0）
STR_TO_NUM_MAP = {
    "mets": {0: 0, 1: 1},
    "joga": {0: 0, 1: 1},
    "lifta": {0: 0, 1: 1},
...
def str_to_numeric(df):
    ...
    for col, map_dict in STR_TO_NUM_MAP.items():
        if col in df.columns:
            ...
            df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)  # 确保数值型
            
            new_missing = df[col].isnull().sum()
            print(f"✅ {col}：原始取值{len(original_values)}种 → 转换后缺失值 {new_missing} 个（原始缺失：{original_missing}）")
            # 打印未匹配值（若有，需确认是否为0/1/空以外的取值）
            unmatched = [v for v in original_values if pd.notnull(v) and v not in map_dict.keys()]
            if unmatched:
                print(f"  ⚠️ 未匹配的取值（已默认填充0）：{unmatched[:5]}...（共{len(unmatched)}种）")
    return df


# ========================
# 3. 检测并删除完全缺失特征（仅删除真正全缺失的特征）
# ========================
...
def main():
    ...
    print(f"已删除的完全缺失特征：{DROP_FULL_MISSING_COLS + full_missing}")
